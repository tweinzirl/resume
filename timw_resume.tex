...
\begin{resume_section}{Skills}
  ...
  \textbf{Programming:}  \= Python (2006--Present), SQL (2017-Present), Scala (2023), R (2015--2017), C (2003--2007) \\* %, Java (2003--2006)
...
\begin{resume_section}{Work Experience}
  ...
  {San Francisco, California}{June 2017 -- Present}
    ...
    \item \texttt{AI}: Led the collaborative prototyping of a \underline{\textbf{LangChain agent}} for automating research analyst tasks.
%for exploring bank data; pitched result to executives of various business lines. 
Developed \underline{\textbf{machine learning models}} (e.g., grading loan applications, client segmentation, fuzzy matching) according to model governance standards and deployed them as self-service web (e.g., Dash) apps.
    \item \texttt{Product operations:} \underline{\textbf{Lead data scientist/architect/engineer}} on the ``daily recommendations pipeline'' providing actionable insights to the banking salesforce. Coordinated with executives, software engineers, and product managers regarding testing/launching new recommendation types. Tracked banker engagement metrics (e.g., click rate) and applied \underline{\textbf{statistical methods}} to measure product efficacy (e.g., on deposit growth). 

    \item \texttt{Quantitative analysis:}
Conducted deep dives and produced data products to help executives understand e.g., client attrition, banker efficacy, and recurring transactions.
% recurring transctions = inflows/outflows from the bank to other banks
Quickly and accurately turned around requests for sales intelligence (e.g., targeted client lists), often on short notice, to enable focused sales initiatives that raised millions of deposit dollars (e.g., the 2023 CD reinstatement campaign raised \underline{\textbf{\$14M}}).

% prospect lists scrubbed of existing clients via fuzzy matching
% example: after gaye left, were people withdrawing money in response
% compare q4b banker efficacy vs the other bankers
...
% (e.g., ``How do I install TensorFlow?'', ``How do I create a Python package?'')
  ...
  {Nottingham, England}{September 2014 -- March 2017}
    \item Produced \underline{\textbf{nine}} (two first-author, one second-author) journal publications by leading research tasks such as parameter estimation with Bayesian Markov Chain Monte Carlo for remote sensing data (\underline{\textbf{$\sim$22,000}} spectra), hypothesis testing \& mathematical modeling, coding data pipelines, and mentoring student researchers. 
